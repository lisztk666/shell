...
nice -n -20 apt install  ntpdate pv s-tui gpart ethtool git  hwloc bridge-utils  btop lnav ntfs-3g progress gddrescue mdadm -y
...
mkdir /etc/sh
cd /etc/sh
wget https://ui.idv.tw/tools/hdsentinel-019c-x64.gz  --no-check-certificate
